...
import { translate } from 'react-jhipster/lib/language/translate';
...
const addNotificationError = (content?: any) => {
  if (content && content.includes("error.")) {
    toast.error(translate(content));
  } else {
    toast.error(content);
  }
};

const addErrorAlert = (message: string, key?: string | undefined, data?: undefined) => {
  key = key ? key : message;
...
export default () => next => action => {
  ...
  if (isRejectedAction(action) && error && error.isAxiosError) {
    if (error.response) {
        ...
        switch (response.status) {
          ...
          case 400:
            if (data?.message && data?.message.includes("error.")) {
              addErrorAlert('Server validation error', data?.message);
            } else {
              addErrorAlert('Server validation error', 'error.validation');
            }
                    ...
                    case 400: {
                      ...
                      if (errorHeader) {
                        ...
                        for (const fieldError of fieldErrors) {
                          ...
                          const fieldName = translate(`gescoApp.${fieldError.objectName}.${convertedField}`);
      ...
      addNotificationError(error.message || 'Unknown error!');
    }
  } else if (error) {
    addNotificationError(error.message || 'Unknown error!');
  }
